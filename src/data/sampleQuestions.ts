...
function makePublicLawChoices(seed: AuthoredQuestionSeed): [string, string, string, string, string] {
  return makeAuthoredChoices(seed).map(expandMarkerChoice) as [string, string, string, string, string];
}
...
function buildAuthoredSubjectQuestions(subject: Subject, seeds: AuthoredQuestionSeed[]): Question[] {
  return seeds.map((seed, index) => ({
    ...
    trapType: seed.trapType,
    questionText: seed.questionText,
    choices: subject === '공법' ? makePublicLawChoices(seed) : makeAuthoredChoices(seed),
    answer: seed.answer,
    explanation: seed.explanation,
